...
import com.ydc.cgj.rental.web.constant.Constants;
...
public class LoginController {
    ...
    public String getLoginParam(Map<String, Object> synMap) {
        try {
            ...
            String redirectUri = StringUtil.isNotEmpty(dd.getRedirectUri()) ? dd.getRedirectUri() : SystemPropertiesConfig.RENTAL_DD_REDIRECTURI;
    ...
    public String doLogin(HttpServletRequest request) {
        logger.info("subject:{},rental.redirect.url.failure:{},rental.redirect.url.succeed:{}","授权成功后登陆",SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE,SystemPropertiesConfig.RENTAL_REDIRECT_URL_SUCCEED);
        try {
            if (StringUtil.isEmpty(request.getParameter("code"))) {
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_404;
            }
            Optional<Dd> optional = Optional.ofNullable(DictionaryUtil.getdingtalkParamByServiceIdentifying(Constants.SERVICEIDENTIFYING)
                    .orElseGet(()->userService.getDdConfigByServiceIdentifying(Constants.SERVICEIDENTIFYING)));
            Dd dd = optional.get();
            ...
            try {
                ...
                if (null == obj1 || obj1.getInteger("errcode") != 0) {
                    ...
                    return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_405;
                }
            } catch (Exception e) {
                logger.error(LoginEnum.LG_406.getMessage(), e);
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_406;
            }
            ...
            if (null == obj2 || obj2.getInteger("errcode") != 0) {
                ...
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_407;
            }
            ...
            if (null == obj3 || obj3.getInteger("errcode") != 0) {
                ...
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_408;
            }
            ...
            if (null != obj4 && obj4.getInteger("errcode") != 0) {
                ...
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_409;
            }
            ...
            if (null == obj6 || obj6.getInteger("errcode") != 0) {
                ...
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_410;
            }
            ...
            if (null == user || StringUtil.isEmpty(user.getId())) {
                return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_416;
//                user = new User();
...
//                user = userService.getUserByLoginName(loginName);
                ...
                if (CodeConstant.NORMAL_STATUS == user.getStatus()) {
                    ...
                    return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_411;
                }
            }
           ...
           logger.info(token);
            ...
            return SystemPropertiesConfig.RENTAL_REDIRECT_URL_SUCCEED + token;
        } catch (DisabledAccountException lae) {
            return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_412;
        } catch (AccountException lae) {
            return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_412;
        } catch (Exception e) {
            logger.error("登陆访问异常", e);
            return SystemPropertiesConfig.RENTAL_REDIRECT_URL_FAILURE + ResultConstant.LOGIN_FAILURE_413;
        }
    }
    ...
    public String getUserInfo( HttpServletResponse response) {
        ...
        user.setVersion(Double.valueOf(ShiroAuthUtil.getRoleVersion()));
    ...
    public String performance(@PathVariable(value = "mobilePhone") String mobilePhone, HttpServletRequest request) {
        try {
            ...
            if ("on".equals(SystemPropertiesConfig.SYS_XINGNENG)) {
                ...
                final ServiceUserRole serviceUserRole = serviceUserRoleService.getServiceUserRoleByUserId(user.getId());
                user.setRoleId(serviceUserRole.getRentalRoleId());
                user.setServiceIdentifying(Constants.SERVICEIDENTIFYING);
     ...
     * @return
     */
    @PostMapping(value = "/role/menus")
    public String getRoleVersion(){
        User user = null;
        try {
            user = WebShiroTokenManager.getUser();
            Map<String,List<String>> listMap = ShiroAuthUtil.responsePermissionsMenus(user.getRoleId(),user.getServiceIdentifying());
            logger.info("subject:{},listMap:{}","角色菜单",JsonUtil.gsonStr(listMap));
            return Result.success(listMap).toJSON();
        }catch (Exception e){
            logger.error("subject:{},userName:{},e:{}","角色菜单异常",user.getUserName(),e);
    ...
    public String responsePermissions(){
        ...
        try {
            user = WebShiroTokenManager.getUser();
            Map<String,List<String>> listMap = ShiroAuthUtil.responsePermissions(user.getRoleId(),user.getServiceIdentifying());
